...
*/
void file_thread_function (int m, string filename, FIFORequestChannel* fifo, BoundedBuffer* Req_Buffer) {
    ...
    fifo->cwrite(buf, msg_size);  					    // writes the packet to the channel

    fifo->cread(&file_size, sizeof(int64_t));        // receives the file length
    ...
    delete fm;
    delete[] buf;
    //cout << endl << "OOOOO file func finished" << endl;
}
...
/* worker_thread
    ...
    send msg across a FIFO channel, collect response
    if DATA:
    - send request across a FIFO channel
    ...
    */
void worker_thread_function (int m, FIFORequestChannel* fifo, BoundedBuffer* Req_Buffer, BoundedBuffer* Rsp_Buffer) {
    while(true) {
        ...
        if (msg_type == DATA_MSG) {
            ...
            // cout << "seconds: " << msg->seconds << endl;
            // cout << endl;
            fifo->cwrite(req_buf, sizeof(datamsg));    // request data from server
            double data_rsp;
            fifo->cread(&data_rsp, sizeof(double));   // server response
            pair<int, double> rsp_pair = make_pair(msg->person, data_rsp);  // std::pair
            //memcpy(req_buf, &rsp_pair, sizeof(pair<int, double>)); // push response to buffer
            //Rsp_Buffer->push(req_buf, m); 
            ...
            fifo->cwrite(req_buf, msg_size);         // get file name
            char* rsp_buf = new char[m];                
            fifo->cread(rsp_buf, m);
...
int main (int argc, char* argv[]) {
    ...
    for (int i = 0; i < w; i++) {
        ...
        chan->cwrite(&msg, sizeof(MESSAGE_TYPE));  // send channel request and get name
		chan->cread(req_buf, sizeof(string));
        string chan_name = (string)req_buf;        
        FIFORequestChannel* newchan = new FIFORequestChannel(chan_name, FIFORequestChannel::CLIENT_SIDE);
        channels_vec.push_back(newchan);         // add new channel 
        delete[] req_buf;
    }
    ...
    if (fileRequest) {
        f_thread[0] = thread(file_thread_function, m, f, chan, &request_buffer);
    }
